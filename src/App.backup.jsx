...
const MANUAL_BRAND_SUGGESTIONS = [
  "Kaziranga",
  "MRF",
  "SS",
  "SG",
...
setActiveTab("dashboard");
  ...
  const handleSave = () => {
    ...
    setEntries(prev => {
      ...
      const existingProfit = existing.profit !== undefined && existing.profit !== null
        ? existing.profit
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        showToast("Could not load CSV import history.", false);
      } finally {
        if (!cancelled) {
  ...
  // ── Import activity calendar dashboard ──────────────────────
  ...
  const importFilesByUploadDate = useMemo(() => {
    const map = {};

    importHistory.forEach(row => {
      const uploadDate = toLocalDateInput(row.imported_at);
      if (!uploadDate) return;

      if (!map[uploadDate]) map[uploadDate] = [];
      map[uploadDate].push(row);
    });
  ...
  const manualActivityByDate = useMemo(() => {
    const map = {};

    manualEntriesAll.forEach(entry => {
  ...
  const selectedUploads = selectedHistoryDate
    ? (importFilesByUploadDate[selectedHistoryDate] || [])
    : importHistory;

  const selectedManualEntries = selectedHistoryDate
    ? (manualActivityByDate[selectedHistoryDate] || [])
    : [];
    ...
    <div style={{ minHeight: "100vh", background: G.cream, fontFamily: "'Segoe UI', system-ui, sans-serif", color: G.ink }}>
      ...
      {activeTab === "import" && (
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 16px" }}>
          ...
          {/* Import Activity Calendar dropdown */}
          <div style={{
            marginTop: 34,
            background: G.card,
            borderRadius: 18,
            ...
            <div style={{
              ...
              >
                {showHistoryDashboard ? "Hide Activity Calendar ▲" : "Show Activity Calendar ▼"}
              </button>
            </div>
                    ...
                    <div style={{
                      ...
                      <div>
                        ...
                        <div style={{ color: G.muted, fontSize: 12, marginTop: 3 }}>
                          {selectedHistoryDate
                            ? new Date(selectedHistoryDate + "T00:00:00").toLocaleDateString("en-US", {
                                weekday: "long",
                                month: "long",
                                day: "numeric",
                                year: "numeric",
      ...
      {activeTab === "dashboard" && (
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "24px 16px" }}>
          ...
          {showManualEntries && (
            <div style={{ background: G.card, borderRadius: 14, padding: "18px 20px", marginBottom: 24, boxShadow: "0 1px 6px #0001", border: "1px solid #E5F2EA" }}>
              ...
              {manualEntriesAll.length === 0 ? (
                <div style={{ textAlign: "center", color: G.muted, fontSize: 13, padding: "28px 0", background: "#F9FBFA", borderRadius: 12 }}>
                ...
                <div style={{ overflowX: "auto", maxHeight: 360, overflowY: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    ...
                    <tbody>
                      {manualEntriesAll.map((entry, i) => (
                        <tr key={entry.id || `${entry.date}-${i}`} style={{ background: i % 2 === 0 ? "#F9FBFA" : G.card, borderBottom: "1px solid #F0F0F0" }}>
